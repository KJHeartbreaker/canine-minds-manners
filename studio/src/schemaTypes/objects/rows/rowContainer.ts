  ...
  preview: {
    ...
    prepare({title, row, disabled, image, ...rowContent}) {
      ...
      if (!previewImage && subs[0]) {
        // Look for first image in rowContent - check carousel, mainImage, or imageButtonCard
        ...
        } else if (firstImage?._type === 'imageButtonCard' && firstImage?.image?.asset) {
          previewImage = firstImage.image
        }
      }
