...
export default function NavbarDropdown({ menuItem, pathname, useSansFont = false }: NavbarDropdownProps) {
    ...
    const mainHref = cta?.landingPageRoute?.slug
        ? (cta.landingPageRoute.slug.startsWith('/') ? cta.landingPageRoute.slug : `/${cta.landingPageRoute.slug}`)
        : cta?.link
            ? (cta.link.startsWith('/') || cta.link.startsWith('http') ? cta.link : `/${cta.link}`)
            : '#'
    ...
    return (
        ...
        >
            ...
            {subnav && subnav.length > 0 && (
                ...
                >
                    {subnav.map((l) => {
                        const subHref = l.landingPageRoute?.slug
                            ? (l.landingPageRoute.slug.startsWith('/') ? l.landingPageRoute.slug : `/${l.landingPageRoute.slug}`)
                            : '#'
