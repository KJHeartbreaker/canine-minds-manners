...
export default function MobileHeaderDropdown({
    ...
    const mainHref = item.cta?.landingPageRoute?.slug
        ? (item.cta.landingPageRoute.slug.startsWith('/') ? item.cta.landingPageRoute.slug : `/${item.cta.landingPageRoute.slug}`)
        : item.cta?.link
            ? (item.cta.link.startsWith('/') || item.cta.link.startsWith('http') ? item.cta.link : `/${item.cta.link}`)
            : '#'
